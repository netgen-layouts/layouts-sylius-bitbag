...
namespace Netgen\Layouts\Sylius\BitBag\Block\BlockDefinition\Configuration\Provider;
...
class BitBagComponentConfigProvider implements ConfigProviderInterface
{
    /**
    ...
    private function resolveViewTypes(string $componentTypeIdentifier): array
    {
        ...
        foreach ($syliusResourceDefaultViewConfig as $config) {
            if (!in_array($componentTypeIdentifier, $this->getConfiguredBitBagComponentIdentifiers($config), true)) {
                continue;
            }
     ...
     * @return string[]
     */
    private function getConfiguredBitBagComponentIdentifiers(array $config): array
    {
        $value = $config['match']['bitbag_component\identifier'] ?? [];
